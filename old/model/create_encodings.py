...
def _zipped_folders_labels_images(training_dir_path, labels):
    return list(
        zip(
            _get_training_dirs(training_dir_path),
            labels,
    ...
    for i in _zipped_folders_labels_images(training_dir_path, labels):
        for fileName in i[2]:
            ...
            if len(imgEncoding) > 1:
                print(
                    "\x1b[0;37;43m"
                    + "More than one face found in {}. Only considering the first face.".format(
                        file_path
                    )
                    + "\x1b[0m"
                )
            if len(imgEncoding) == 0:
                print(
                    "\x1b[0;37;41m"
                    + "No face found in {}. Ignoring file.".format(file_path)
                    + "\x1b[0m"
                )
            else:
...
if os.path.isfile(encoding_file_path):
    ...
    backup_file_path = "{}.bak".format(encoding_file_path)
    if os.path.isfile(backup_file_path):
        os.remove(backup_file_path)

    os.rename(encoding_file_path, backup_file_path)
...
print(
    "\x1b[6;30;42m"
    + "Training Image's encodings saved in {}".format(encoding_file_path)
    + "\x1b[0m"
)
