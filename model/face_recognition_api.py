...
face_recognition_model = os.path.join(
    os.getcwd(), "models", "dlib_face_recognition_resnet_model_v1.dat"
)
...
def _trim_rect_tuple_to_bounds(rect, image_shape):
    ...
    return (
        max(rect[0], 0),
        min(rect[1], image_shape[1]),
        min(rect[2], image_shape[0]),
        max(rect[3], 0),
    )
...
def load_image_file(filename, mode="RGB"):
    ...
    if img.size[0] > 800 or img.size[1] > 800:
        baseheight = 500
        if img.size[0] > img.size[1]:  # Width is greater than height
            wpercent = baseheight / float(img.size[0])
            hsize = int((float(img.size[1]) * float(wpercent)))
            img = img.resize((baseheight, hsize), Image.LANCZOS)
